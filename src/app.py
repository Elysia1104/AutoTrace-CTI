...
def extract_all_iocs(text):
    ...
    reg_pattern = r'(?:HKLM|HKCU)\\[a-zA-Z0-9_\\-]+'
    ...
    iocs = {
        ...
        "registry": list(set(re.findall(reg_pattern, text, re.IGNORECASE)))
    }
    ignored_exts = ('.png', '.json', '.py', '.html')
    iocs["domains"] = [
        d for d in iocs["domains"] if not d.endswith(ignored_exts)
    ]
    return iocs
...
def build_stix_indicators(iocs):
    ...
    for domain in iocs["domains"]:
        try:
            ...
            stix_objects.append(
                {"type": "Domain", "value": domain, "obj": ind}
            )
        except Exception:
            stix_objects.append(
                {"type": "Domain", "value": domain, "obj": None}
            )

    for hash_val in iocs["sha256"]:
        try:
            ind = Indicator(
                name=f"SHA256 - {hash_val[:8]}...",
                pattern=f"[file:hashes.'SHA-256' = '{hash_val}']",
                pattern_type="stix"
            )
            stix_objects.append(
                {"type": "SHA256", "value": hash_val, "obj": ind}
            )
        except Exception:
            stix_objects.append(
                {"type": "SHA256", "value": hash_val, "obj": None}
            )
    ...
    for reg in iocs["registry"]:
        ...
        try:
            ind = Indicator(
                name="Registry Key",
                pattern=f"[windows-registry-key:key = '{escaped_reg}']",
                pattern_type="stix"
            )
            stix_objects.append({"type": "Registry", "value": reg, "obj": ind})
        except Exception:
            stix_objects.append(
                {"type": "Registry", "value": reg, "obj": None}
            )
...
def correlate_and_build_graph(stix_indicators, net_file, proc_file):
    ...
    for item in stix_indicators:
        ...
        mitre = MITRE_MAPPING.get(
            ioc_type, {"id": "T1000", "name": "Unknown"}
        )

        if len(val) > 15:
            node_id = f"{ioc_type}: {val[:12]}..."
        else:
            node_id = f"{ioc_type}: {val}"

        node_lbl = f"{ioc_type}: {val[:15]}\n[{mitre['id']}]"
        G.add_node(node_id, label=node_lbl, color="#ffa64d")
        ...
        if ioc_type == "IPv4":
            ...
            if not matched:
                hits.append({
                    ...
                    "MITRE_ID": mitre["id"],
                    "MITRE_Name": mitre["name"],
                    "Artifact_Detail": "Extracted Indicator (No Active Socket)"
                })
        else:
            hits.append({
                "Type": ioc_type,
...
def generate_pdf_report(hits, output_pdf_path):
    ...
    story.append(
        Paragraph("AutoTrace-CTI: Forensic & MITRE Summary", title_style)
    )
    story.append(Spacer(1, 10))

    summary_text = (
        f"<b>Execution Summary:</b> Identified <b>{len(hits)}</b> indicators."
    )
    ...
    if hits:
        for h in hits:
            val_clean = str(h.get("Value")).replace(
                "&", "&amp;"
            ).replace(
                "<", "&lt;"
            ).replace(
                ...
                <div class="col-md-5">
                    <div class="card bg-secondary text-white p-3 mb-3">
                        ...
                        <form action="/analyze" method="post">
                            <textarea name="cti_text" class="form-control mb-3"
                              rows="8" placeholder="Paste report..."></textarea>
                            <button type="submit"
                              class="btn btn-danger w-100 mb-2">
                              Run Correlation & MITRE Mapping
                            </button>
                        </form>
                        <a href="/download-pdf" target="_blank"
                          class="btn btn-primary w-100">
                          View SOC Forensic PDF Report
                        </a>
                    </div>
                </div>
                <div class="col-md-7">
                    <div class="card bg-secondary text-white p-3">
                        ...
                        <iframe src="/graph" width="100%" height="480px"
...
async def get_graph():
    ...
    return HTMLResponse(
        content="<h5 class='p-3 text-muted'>No graph generated yet.</h5>"
    )
